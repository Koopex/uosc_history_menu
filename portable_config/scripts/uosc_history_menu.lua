...
elseif o.language == 'zh' then
	t = {
		del = '删除该记录：按下“Delete”',
		title_all = '播放记录（全部）',
		title_deduplicated = '播放记录（去重）',
		title_folders = '播放记录（文件夹）',
...
local function readLogFile() -- 读取 JSON Lines 格式日志并转为表格
    local array = {}
    local file = io.open(o.log_path, 'r')
    
    if file then
        for line in file:lines() do
            local ok, data = pcall(utils.parse_json, line)
            if ok and data then
                -- 将 JSON 字段映射到表格结构（保持与原字段命名一致）
                table.insert(array, {
                    date_time = data.system_time,           -- 1 日期时间
                    media_title = data.media_title,         -- 2 媒体标题
                    file_name = data.file_name,             -- 3 文件名
                    path = data.path,                      -- 4 文件路径
                    upper_path = data.upper_path,          -- 5 上一级路径
                    parent_folder = data.parent_folder,    -- 6 文件夹名
                    position_in_folder = data.position_in_folder, -- 7 在目录中的位置
                    position_sec = data.duration,          -- 8 播放进度(秒)
                    position_for = data.formatted_duration, -- 9 播放进度(时分秒)
                    duration_for = data.total_duration,     -- 10 视频时长(时分秒)
                    percent = data.progress_percent        -- 11 播放进度(百分比)
                })
            else
                mp.msg.warn("Failed to parse JSON line: " .. (line or "nil"))
            end
        end
        file:close()
    else
		...
		if o.menu_filter == 'folders' then
			for i = #arry, 1, -1 do
				...
				if v.position_in_folder ~= 'url' then
					local parent_folder = v.upper_path:match('([^\\]+)$')
					if parent_folder ~= nil and string.find(parent_folder, '^[Ss]eason[^%a%d]*%d+') ~= nil then
						x = v.upper_path:gsub('\\[^\\]*$', '')
					else
						x = v.upper_path
					end
				else
					x = v.position_in_folder
				end
				if not seen[x] then
...
local function openMenu(num) -- 打开菜单
	...
	local file = io.open('D:\\Users\\Desktop\\log.json', "w")
	file:write(utils.format_json(menu_props)) 
	file:close()
...
local function writeLog() -- 写入日志（JSON Lines 格式）
    if next(log_part) ~= nil then
        -- 构造结构化 JSON 对象
        local entry = {
            system_time = log_part[1],          -- 系统时间
            media_title = log_part[2],          -- 媒体标题
            file_name = log_part[3],            -- 文件名
            path = log_part[4],                 -- 完整路径
            upper_path = log_part[5],           -- 上级路径
            parent_folder = log_part[6],        -- 父文件夹名
            position_in_folder = log_part[7],   -- 在文件夹中的位置
            duration = time_pos,                -- 当前播放时长（秒）
            formatted_duration = formatTime(time_pos),               -- 格式化的时长（HH:MM:SS）
            total_duration = formatTime(log_part[8]),                -- 总时长（HH:MM:SS）
            progress_percent = formatPercent(time_pos, log_part[8])  -- 播放进度百分比
        }
        
        -- 转换为 JSON 字符串并写入文件
        local json_line = utils.format_json(entry) .. "\n"
...
local function deleteLog(type, target)
    -- 读取现有日志
    ...
    if type == 'index' then
        -- 按索引删除
        for i, entry in ipairs(old_log) do
            if i ~= target then
                table.insert(new_log, entry.raw_data or {
                    system_time = entry.date_time,
                    media_title = entry.media_title,
                    file_name = entry.file_name,
                    path = entry.path,
                    upper_path = entry.upper_path,
                    parent_folder = entry.parent_folder,
                    position_in_folder = entry.position_in_folder,
                    duration = entry.position_sec,
                    formatted_duration = entry.position_for,
                    total_duration = entry.duration_for,
    ...
    elseif type == 'path' then
        -- 按路径删除
        for _, entry in ipairs(old_log) do
    ...
    elseif type == 'upper_path' then
        -- 按上级路径删除
        if string.find(target:match('([^\\]+)$'), '^[Ss]eason[^%a%d]*%d+') ~= nil then
			target = target:gsub('\\[^\\]*$', '')
			for _, entry in ipairs(old_log) do
				if entry.upper_path:gsub('\\[^\\]*$', '') ~= target then
					table.insert(new_log, entry.raw_data or {
						system_time = entry.date_time,
						media_title = entry.media_title,
						file_name = entry.file_name,
			...
			for _, entry in ipairs(old_log) do
				if entry.upper_path ~= target then
					table.insert(new_log, entry.raw_data or {
						system_time = entry.date_time,
						media_title = entry.media_title,
						file_name = entry.file_name,
    ...
    elseif type == 'media_title' then
        -- 按媒体标题删除（特殊处理URL）
        for _, entry in ipairs(old_log) do
            if not (entry.media_title == target and entry.position_in_folder == 'url') then
                table.insert(new_log, entry.raw_data or {
                    system_time = entry.date_time,
                    media_title = entry.media_title,
                    file_name = entry.file_name,
    ...
    for _, entry in ipairs(new_log) do
        -- 直接使用原始数据或转换后的数据
        local json_line = utils.format_json(entry) .. "\n"
        content = content .. json_line
    end
    
    -- 安全写入文件（覆盖模式）
    local file, err = io.open(o.log_path, "w")
    if not file then
        mp.msg.error("Failed to open log file for writing: " .. (err or "unknown error"))
        return false
    end
    ...
    if not success then
        mp.msg.error("Failed to write log file: " .. (write_err or "unknown error"))
        return false
    end
